...
class GraphMetrics(MetricsBase):
    ...
    def epoch_metrics(self):
        ...
        metrics = {}
        metrics['acc/train'] = 100 * self.train_correct / self.train_total
        metrics['acc/valid'] = 100 * self.valid_correct / self.valid_total
        for i, name in enumerate(self.class_names):
            metrics[f'acc_class/train/{name}'] = 100 * self.train_class_correct[i] / self.train_class_total[i] if self.train_class_total[i] > 0 else 0
            metrics[f'acc_class/valid/{name}'] = 100 * self.valid_class_correct[i] / self.valid_class_total[i] if self.valid_class_total[i] > 0 else 0
        metrics['time/train'] = self.train_end - self.epoch_start
        metrics['time/valid'] = self.valid_end - self.train_end
        return metrics
