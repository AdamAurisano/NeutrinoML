...
class Fish(ME.MinkowskiNetwork):
    def __init__(self, block, D, A, num_cls=1000, num_down_sample=5, num_up_sample=3, trans_map=(2, 1, 0, 6, 5, 4),
        ...
        self.fish = self._make_fish(D, A, network_planes[0])
    ...
    def _make_fish(self, D, A, in_planes):
        ...
        # cated_planes = [in_planes] * self.depth -> every layer is multiplied by the depth
        # fish = [], empty list
        # make this loop to split into three parts where the self.depth will loop over network planes which has a list of module list in a row. 
        cated_planes, fish = [in_planes] * self.depth, []
        # adding things to fish and returning at the end list of modules
