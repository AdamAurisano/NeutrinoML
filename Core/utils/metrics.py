...
def metrics_semantic_segmentation(batch_target, batch_output, class_names=None):
    ...
    if class_names is not None:
        if len(class_names) != batch_output.shape[1]:
          raise Exception(f'Number of class names ({len(class_names)}) does not match shape of network output ({batch_output.shape[1]})!')
            ...
            metrics['batch']['IndivAcc/acc_indiv'] = [ 100*((w_pred[correct]==i).sum().float()/(w_true==i).sum().float()).item()\
