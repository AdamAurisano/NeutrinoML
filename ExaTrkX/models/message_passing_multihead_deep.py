...
class GNNDeepMultiHead(nn.Module):
  def __init__(self, input_dim=2, output_dim=4, hidden_dim=8, n_iters=3,
               hidden_activation=nn.Tanh, how='standard',
               stacks=True, edgefeats=False, classify_node=False, **kwargs):
  ...
  def forward(self, data):
    ...
    dev = data.x.device
    # mem("at model start")
    #print("data.x shape: " + str(data.x.shape))
    #print("data.y shape: " + str(data.y.shape))
    ...
    if self.classify_node: return self.out_net(H).squeeze()
    else: return self.edge_network(data).squeeze()
