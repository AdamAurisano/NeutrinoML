...
class GNNSegmentClassifier(nn.Module):
  def __init__(self, neighbours=5, input_dim=2, hidden_dim=8, n_iters=3, hidden_activation=nn.Tanh, transforms=None, **kwargs):
    super(GNNSegmentClassifier, self).__init__()
    self.transforms = tf.Compose([getattr(dune_tf,tfname)() for tfname in transforms]) if transforms is not None else None
  ...
  def forward(self, data):
    ...
    start = time()
    data.edge_index = knn_graph(data.pos, self.neighbours)
    print(f'KNN took {time()-start}')
    ...
    return self.output_network(data.x).squeeze(-1)
