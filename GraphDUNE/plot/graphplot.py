...
def plot_edge_score(trainer, graph):
  ...
  trainer.model.eval()
  y = trainer.model(graph.x.to(trainer.device)).argmax(dim=1)
  lines = get_lines(graph, y)
...
def plot_edge_diff(trainer, graph):
  fig, ax = plt.subplots(figsize=[16,9])
  trainer.model.eval()
  y = trainer.model(g.x.to(trainer.device)).argmax(dim=1)
  y = (y == graph.y)
  lines = get_lines(graph, y)
  for lc in lcs: ax.add_collection(lc)
  ax.autoscale()
