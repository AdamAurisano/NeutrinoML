...
class Pixel():
    ...
    for idx in range(En.shape[0]):
      for jdx in range(len(En[idx])):
        if abs(trackId[idx][jdx]) in trueEmshowers:
          GT[idx,7] += En[idx][jdx] / sum(En[idx])
    return GT
  ...
  def classifier(dic):
    ...
    GT = np.zeros([coo.shape[0],8]).astype(np.float32)
    for idx in range(coo.shape[0]):
     
      if len(pdg[idx]) == 0:
        GT[idx,0] = 1     ### NOISE 
      else: norm = 1/sum(energy[idx])
      for jdx in range(len(pdg[idx])):
        ...
        # EM activity
          ...
          GT[idx,2] += energy[idx][jdx]
        elif trackId[idx][jdx] > 0:
          # Michel electrons + positrons
          if pdg[idx][jdx] == 98 or pdg[idx][jdx] == 99:
            GT[idx,6] += energy[idx][jdx]
          # Highly ionising particles
          elif PDGID(pdg[idx][jdx]).is_nucleus == True or abs(pdg[idx][jdx]) == 321 or 'Sigma' in Particle.from_pdgid(pdg[idx][jdx]).name:
