...
def GetShowers(pdg, trackId):
  ...
  return tuple(key for key, val in count.items() if val > 55)
    ...
    for jdx in range(len(pdg[idx])):
      ...
      # Electron shower 
        ...
        # Photon shower
          ...
          y_true[idx,2] += energy[idx][jdx] # Diffuse energy deposition if ten or fewer hits
          proc[idx,2] = str(process[idx][jdx])
      # Delta rays
      elif trackId[idx][jdx] < 0 and (abs(pdg[idx][jdx]) == 13 or abs(pdg[idx][jdx]) ==211): # Delta rays 
